...
class PersonalConditions extends Component {
    ...
    handlePersonalConditionsChange = (event, section, indexCondition, answerType) => {
        ...
        this.props.onPersonalConditionsChange(event, section, indexCondition, answerType);
            ...
            c.conditions.map((q, index2) => {
                ...
                rows.push(
                    <Row className='p-2'>
                        ...
                        <Col>
                            {(q.answerType === 'string')?
                                <input value={q.answer} onChange={(e) => this.handlePersonalConditionsChange(e, index, index2, q.answerType)} />
                                :
                                null
                            }
                            {(q.answerType === 'boolean') ?
                                <input type='checkbox' checked={q.answer} onChange={(e) => this.handlePersonalConditionsChange(e, index, index2, q.answerType)} />
                                :
                                null
                            }
                        </Col>                        
    ...
    handlePersonalConditionsChange = (event, section, indexCondition, answerType) => {
        ...
        switch (answerType){
            ...
            case 'string':
                ...
                break;
            default:
                console.log('handlePersonalConditionsChange undefined ansType=',answerType); 
        }
    }
        ...
        axios
            ...
            .catch((error) => {
                ...
                if (typeof(error.response.status) == 'undefined'){
                    console.log('Consultations error:', error.response);
                    this.props.history.push('/login');
